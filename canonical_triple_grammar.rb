...
module Iqvoc
  module RDFAPI
    module CanononicalTripleGrammar
      def r_name(context = '')
      ...
      def r_namespaced_origin(context = '')
        /(?<#{context}Prefix> #{r_name(context)} ) :
         (?<#{context}Origin> #{r_name(context)} )/x
      end
      ...
      def r_lang_string(context = '')
        ...
        /"#{r_string(my_context)}" (@#{r_language(my_context)})?/x
      end
      ...
      def r_subject(context = '')
        ...
        /(?<#{my_context}> #{r_namespaced_origin(my_context)} )/x
      end
