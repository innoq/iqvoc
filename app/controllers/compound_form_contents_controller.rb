class CompoundFormContentsController < ApplicationController
  def create
    ...
    label = Iqvoc::Label.base_class.find(params[:id])
    label_new_version = Iqvoc::Label.base_class.new_version(label.origin).first
  ...
  def destroy
    ...
    versioned_label = Iqvoc::Label.base_class.new_version(compound_form_content.label.origin).first
