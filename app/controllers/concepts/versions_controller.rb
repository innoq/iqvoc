...
class Concepts::VersionsController < ApplicationController
    ...
    ActiveRecord::Base.transaction do
      if current_concept.blank? || current_concept.destroy
        ...
        if new_version.valid_with_full_validation?
          ...
          redirect_to concept_path(:lang => I18n.locale, :id => new_version)
        else
          flash[:error] = t("txt.controllers.versioning.merged_publishing_error")
          redirect_to concept_path(:published => 0, :id => new_version, :lang => I18n.locale)
        end
      else
  ...
  def branch
    ...
    redirect_to edit_concept_path(:published => 0, :id => new_version, :lang => I18n.locale, :check_associations_in_editing_mode => true)
  end
  ...
  def lock
    ...
    redirect_to edit_concept_path(:published => 0, :id => new_version, :lang => I18n.locale)
  end
  ...
  def consistency_check
    ...
    if concept.valid_with_full_validation?
      ...
      redirect_to edit_concept_path(:published => 0, :id => concept, :lang => I18n.locale, :full_consistency_check => "1")
    end
  end
  ...
  def to_review
    ...
    redirect_to concept_path(:published => 0, :lang => I18n.locale, :id => concept)
  end
