  ...
  module Validations
    ...
    def pref_label_in_primary_thesaurus_language
      if @full_validation
        ...
        if self.pref_labels.empty?
          errors.add :base, I18n.t('txt.models.concept.no_pref_label_error')
        elsif not self.pref_labels.find {|l| l.language.to_s == primary_language }
    ...
    def unique_pref_label
      if @full_validation
        ...
        # language and value
        conflicting_pref_labels = self.pref_labels.select do |l|
        ...
        if conflicting_pref_labels.any?
          if conflicting_pref_labels.one?
            lbl = conflicting_pref_labels.last.value
          else
            lbl = conflicting_pref_labels.map(&:value).join(', ')
          end
          errors.add :base, I18n.t('txt.models.concept.pref_label_not_unique', :label => lbl)
        end
      end
    end
    ...
    def valid_rank_for_ranked_relations
      if @full_validation
        self.relations.each do |relation|
          if relation.class.rankable? and (0..100).exclude?(relation.rank)
