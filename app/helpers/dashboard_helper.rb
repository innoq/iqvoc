  ...
  def sorting_controls_for(name)
    content_tag :div, class: 'sorting-controls' do
      link_to(icon('arrow-circle-o-up', 'sorting-arrow'),
        dashboard_path(request.query_parameters.slice(:sort).merge(sort: "#{name} ASC") { |key,oldval,newval| oldval.include?(newval) ? oldval : "#{oldval},#{newval}" })) +
        link_to(icon('arrow-circle-o-down', 'sorting-arrow'),
        dashboard_path(request.query_parameters.slice(:sort).merge(sort: "#{name} DESC") { |key,oldval,newval| oldval.include?(newval) ? oldval : "#{oldval},#{newval}" }))
    end
  end
